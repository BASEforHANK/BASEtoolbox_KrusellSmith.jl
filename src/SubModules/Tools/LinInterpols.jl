...
function mylinearinterpolate2(
    ...
    @views for i in eachindex(xeval1)
        ...
        dist = xgrd1[iL.+1] .- xgrd1[iL]
        if dist > 0
            weight1[i] = copy((xi .- xgrd1[iL]) ./ dist)
        elseif dist == 0
            weight1[i] = 0.0
        else
            @error "xgrd1[iL.+1] < xgrd1[iL]"
        end
            ...
            for j in eachindex(xeval2)
                ...
                for k in eachindex(xeval1)
                    ...
                    yeval[k, j, l] =
                        ...
                        w1r .* (
                            w2l .*
                            (w3l .* ygrd[i1+1, i2, i3] + w3r .* ygrd[i1+1, i2, i3+1]) +
                            w2r .*
                            (w3l .* ygrd[i1+1, i2+1, i3] + w3r .* ygrd[i1+1, i2+1, i3+1])
                        )
                end
            end
        end
...
function poly_coeff_Akima2(xgrd1::Vector, xgrd2::Vector, F::AbstractMatrix)
    ...
    txy =
        (
            wx[3:nx+2, :] .*
            (wy[:, 3:ny+2] .* mxy[2:nx+1, 2:ny+1] .+ wy[:, 1:ny] .* mxy[2:nx+1, 3:ny+2]) .+
            wx[1:nx, :] .*
            (wy[:, 3:ny+2] .* mxy[3:nx+2, 2:ny+1] .+ wy[:, 1:ny] .* mxy[3:nx+2, 3:ny+2])
...
function poly_coeff_Akima3(xgrd1::Vector, xgrd2::Vector, xgrd3::Vector, F::AbstractArray)
    ...
    ty =
        ...
        (wy[:, 3:ny+2, :] + wy[:, 1:ny, :])
    tz =
        (wz[:, :, 3:nz+2] .* mz[:, :, 2:nz+1] + wz[:, :, 1:nz] .* mz[:, :, 3:nz+2]) ./
        (wz[:, :, 3:nz+2] + wz[:, :, 1:nz])
