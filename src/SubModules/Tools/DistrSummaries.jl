...
@doc raw"""
    distrSummaries(cdf,c_a_star,c_n_star,n_par,inc,incgross,m_par)
...
# Arguments
- `cdf`: joint distribution over bonds, capital and income ``(m \times k \times y)``
...
function distrSummaries(
    ...
    cdf_m = sum(cdf, dims = 2)[:]
    pdf_y = cdf[end, :]
    distr_m = [cdf_m[1]; diff(cdf_m)]
    distr = zeros(eltype(cdf), size(cdf))
    distr[1, :] .= cdf[1, :]
    distr[2:end, :] .= diff(cdf, dims = 1)
    ...
    return cdf_m, pdf_y, TOP10Wshare, TOP10Ishare, giniwealth, giniconsumption, sdlogy
end
