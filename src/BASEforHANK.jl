...
function call_find_steadystate(m_par)
    ...
    KSS, VmSS, CDFSS, n_par, m_par = find_steadystate(m_par)

    return SteadyStateStruct(KSS, VmSS, CDFSS, n_par)
end
...
function call_prepare_linearization(ss, m_par)
    ...
    CDF_m,
    PDF_y,
    distrSS = prepare_linearization(ss.KSS, ss.VmSS, ss.CDFSS, ss.n_par, m_par)
