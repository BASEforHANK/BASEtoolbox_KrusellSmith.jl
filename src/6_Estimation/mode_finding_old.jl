...
function mode_finding(sr, lr, m_par, e_set, par_start)
    ...
    #     IRFtargets = Array{Float64}(undef, e_set.irf_horizon, length(select_variables),length(shocks_selected))
    #     weights    = Array{Float64}(undef, e_set.irf_horizon, length(select_variables),length(shocks_selected))
    ...
    #             countj += 1
    #             IRFtargets[:, countj, counti] = Data_temp[Data_temp[:, :pointdum] .& (Data_temp[:, :shock].==i), j]
    #             weights[:, countj, counti]    = Data_temp[.!Data_temp[:, :pointdum] .& (Data_temp[:, :shock].==i), j]
    #         end
    #     end
    ...
    #     # Optimization
    #     # Define objective function
    #     Laux(pp)  = -irfmatch(par, IRFtargets, weights, isstate, indexes_sel_vars, priors, sr, lr, m_par, e_set)[3]
    ...
    #     # Define closure of objective function
    #     LL(pp)  = -irfmatch(par, IRFtargets, weights, isstate, indexes_sel_vars, priors, sr, lr, m_par, e_set)[3]
    ...
    #     LL_final(pp)  = -irfmatch(par, IRFtargets, weights, isstate, indexes_sel_vars, priors, sr, lr, m_par, e_set)[3]
