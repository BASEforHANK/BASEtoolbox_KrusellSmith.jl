...
function prepare_linearization(KSS, VmSS, distrSS, n_par, m_par)
    ...
    KSS, TransitionMatSS, c_n_starSS, m_n_starSS, VmSS, distrSS =
        Ksupply(rSS, n_par, m_par, VmSS, distrSS, inc)

    VmSS .*= rSS


    # Produce distributional summary statistics
    distr_mSS, distr_ySS, TOP10WshareSS, TOP10IshareSS, GiniWSS, GiniCSS, sdlogySS =
        distrSummaries(distrSS, c_n_starSS, n_par, inc, m_par)
    ...
    price_vectorSS = [rSS, wSS][:]

    indm, _ = first_stage_reduction(
        VmSS,
        TransitionMatSS,
        NSS,
        m_n_starSS,
    ...
    # ------------------------------------------------------------------------------
    CDFSS = cumsum(cumsum(distrSS, dims = 1), dims = 2) # Calculate CDF from PDF
    distr_mSS = sum(distrSS, dims = 2)[:, 1]            # Marginal distribution (pdf) of liquid assets
    distr_ySS = sum(distrSS, dims = 1)[1, :]            # Marginal distribution (pdf) of income
    ...
    @set! n_par.copula_marginal_m =
        copula_marg_equi(distr_mSS, n_par.grid_m, n_par.nm_copula)
    @set! n_par.copula_marginal_y =
        copula_marg_equi_y(distr_ySS, n_par.grid_y, n_par.ny_copula)
