...
function compute_reduction(sr, lr, m_par, shock_names)
    ...
    indexes_r = produce_indexes(n_par, keepV[keepV], keepD[keepD]) # arbitrary splitup of underlying factors from value functions to indexes in reduced model
    @set! n_par.ntotal_r = n_par.nstates_r + n_par.ncontrols_r

    block = Array{Vector}(undef, 5)
    block_r = Array{Vector}(undef, 5)
    indexes = sr.indexes
    block[1] = [indexes.distr_m; indexes.distr_y]
    block[3] = indexes.COP[end]+1:indexes.Vm[1]-1
    block[2] = indexes.COP
    block[4] = indexes.Vm
    block[5] = indexes.Vm[end]+1:n_par.ntotal
    block_r[1] = [indexes_r.distr_m; indexes_r.distr_y]
    block_r[3] = indexes_r.COP[end]+1:indexes_r.Vm[1]-1
    block_r[2] = indexes_r.COP
    block_r[4] = indexes_r.Vm
    block_r[5] = indexes_r.Vm[end]+1:n_par.ntotal_r
